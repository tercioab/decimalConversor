...
export default function Page() {
	...
	function switchNumberTypes(type) {
		switch (type) {
			case "Octal":
				return 8;
			case "Hexadecimal":
	...
	function binario(number, type) {
		...
		let num = number;
		let ar = [];
		let typeInNumber = switchNumberTypes(type);
	...
	return (
		<div className='flex items-center justify-center h-screen'>
			<div className='max-w-sm p-6 bg-white border border-slate-200 rounded-lg shadow dark:bg-slate-800 dark:border-slate-700'>
				<p className='text-center text-2xl font-extrabold mt-10 text-transparent bg-clip-text bg-gradient-to-r to-sky-400 from-indigo-500'>
				...
				<p className='text-center text-slate-800 text-sm font-medium mt-10'>
					feito por{" "}
					<a
